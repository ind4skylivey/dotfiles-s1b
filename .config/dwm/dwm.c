...
enum { SchemeNorm, SchemeSel, SchemeUrg, SchemeTitle, SchemeLayout }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		tw = statusw; /* width pre-computed in updatestatus (no color codes) */
		char *ts = stext;
		char *tp = stext;
		char ctmp;
		Clr statusscheme[2];
		...
		/* status2d: parse ^c#rrggbb^ (fg) ^b#rrggbb^ (bg) ^d^ (reset) */
		while (1) {
			if ((*ts == '^') && (*(ts+1) == 'c' || *(ts+1) == 'b' || *(ts+1) == 'd') ) {
				/* flush text before escape */
				ctmp = *ts; *ts = '\0';
				if (*tp) {
					int segw = TEXTW(tp) - lrpad;
					drw_text(drw, cx, 0, segw + lrpad, bh, lrpad/2, tp, 0);
					cx += segw;
				}
				*ts = ctmp;
				if (*(ts+1) == 'd') {
...
shiftview(const Arg *arg)
{
	...
	shift = (arg->i > 0 ? arg->i : -arg->i) % LENGTH(tags);
	if (shift == 0)
		return;

	tagmask = selmon->tagset[selmon->seltags];
	if (arg->i > 0)
		a.ui = (tagmask << shift) | (tagmask >> (LENGTH(tags) - shift));
	else
		a.ui = (tagmask >> shift) | (tagmask << (LENGTH(tags) - shift));
...
shifttag(const Arg *arg)
{
	...
	newtags = c->tags;
	if (arg->i > 0)
		newtags = (newtags << shift) | (newtags >> (LENGTH(tags) - shift));
	else
		newtags = (newtags >> shift) | (newtags << (LENGTH(tags) - shift));
	...
	if (XineramaIsActive(dpy)) {
		...
		for (i = 0, m = mons; i < nn && m; m = m->next, i++)
			...
			|| unique[i].width != m->mw || unique[i].height != m->mh)
			{
				dirty = 1;
				m->num = i;
			/* show bar only on primary monitor (num == 0) */
			if (i != 0) {
				m->showbar = 0;
				for (j = 0; j <= LENGTH(tags); j++)
					m->pertag->showbars[j] = 0;
			}
